...
async function getArticlesHandler(req, res) {
    const articles = await Article.find(req.query.public ? { public: req.query.public === "true" ? true : false } : null ).lean().exec();
...
async function fetchAllArticles(req, res) {
    ...
    const fileData = fs.readFileSync(path.join(__dirname, '..', 'articles.json'), 'utf8');
    const localeArticles = JSON.parse(fileData).articles;
    res.json([...articles, ...localeArticles])
...
module.exports = { createArticle, getArticleHandler, getArticlesHandler, getArticlesFromLocal, getArticleFromLocal, updateArticleHandler, deleteArticleHandler, fetchAllArticles }